...
def create_parser():
    ...
    parser_d = subparsers.add_parser(
        'baseline',
        help='Compare all tests in the benchmark against a baseline')
    baseline = parser_d.add_argument_group(
        'benchmark', 'The benchmark')
    baseline.add_argument(
        'test_benchmark',
        metavar='test_benchmark',
        type=argparse.FileType('r'),
        nargs=1,
        help='A benchmark executable or JSON output file')
    baseline.add_argument(
        'baseline_test',
        metavar='baseline_test',
        type=str,
        nargs=1,
        help='The test to use as a baseline')
...
def main():
    ...
    if args.mode == 'benchmarks':
        ...
        test_baseline = args.test_benchmark[0].name
        maintest = args.baseline_test[0]
        test_contender = None
        filter_baseline = None
        filter_contender = None
        
        description = 'Comparing %s (from %s) against the rest of the benchmarks' % (maintest, test_baseline)
    else:
        # should never happen
    ...
    print(args.display_aggregates_only, args.utest, args.utest_alpha, args.color, maintest)
    # Diff and output
    output_lines = gbench.report.generate_difference_report(
        json1, json2, args.display_aggregates_only,
        args.utest, args.utest_alpha, args.color, maintest)
    print(description)
