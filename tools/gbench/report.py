...
def partition_benchmark(json1, maintest):
    """
    While preserving the ordering, find benchmarks with the same names in
    both of the inputs, and group them.
    (i.e. partition/filter into groups with common name)
    """
    json1_unique_names = get_unique_benchmark_names(json1)
    names = [name for name in json1_unique_names]
    assert(len(names) == len(set(names)))
    time_unit = None
    # Pick the time unit from the first entry of the lhs benchmark.
    # We should be careful not to crash with unexpected input.
    for x in json1['benchmarks']:
        if (x['name'] == maintest and is_potentially_comparable_benchmark(x)):
            time_unit = x['time_unit']
            break
    if time_unit is None:
        raise Exception("Could not determine time_unit for %s" % maintest)
    partitions = []
    seen_suffixes = set()
    suffixes = [x[len(maintest):] for x in names
                if x.startswith(maintest)
                    and x[len(maintest):] not in seen_suffixes
                    and (seen_suffixes.add(x[len(maintest):]) or True)]
    used = set()
    for suffix in suffixes[::-1]:
        lhs = [x for x in json1['benchmarks'] if x['name'] == maintest + suffix and
                x['time_unit'] == time_unit and x['name'] not in used]
        if len(lhs) == 0:
            continue
        for name in (x['name'] for x in lhs):
            used.add(name)
        for name in (x for x in names if x.endswith(suffix) and not x.startswith(maintest)):
            # Filter by name and time unit.
            # All the repetitions are assumed to be comparable.
            rhs = [x for x in json1['benchmarks'] if x['name'] == name and
                   x['time_unit'] == time_unit and x['name'] not in used]
            for name in (x['name'] for x in rhs):
                used.add(name)
            partitions.append([lhs, rhs])
    return partitions[::-1]
...
def generate_difference_report(
    ...
    if maintest is None:
        ...
        partitions = partition_benchmark(json1, maintest)
    for partition in partitions:
        ...
        for i in range(min(len(partition[0]), len(partition[1]))):
            ...
            if display_aggregates_only and 'run_type' in bn and 'run_type' in other_bench:
                print(bn['run_type'], other_bench['run_type'])
